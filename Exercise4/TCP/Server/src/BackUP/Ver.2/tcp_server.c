...
int HandleClient(int SourceSock,int ServerSock ,int MaxSockNumber)
{
	char MessageBuffer[BUFFSIZE];	/* Send or Received internet message buffer */
	char TempMessageBuffer[BUFFSIZE];
	...
	MessageBufferSize = strlen(TempMessageBuffer);

	for (SockIndex = ServerSock+1 ; SockIndex < MaxSockNumber+1; SockIndex++)
	{
		/* Send back received data */
		if (send(SockIndex, TempMessageBuffer, MessageBufferSize, 0) != MessageBufferSize)
		{
		...
		for (SockIndex = 0; SockIndex < (MaxSockNumber+1); SockIndex++)
		{
			...
			if (FD_ISSET(SockIndex, &ReadSocks)) /* Check SockIndex */
			{
				...
				else
				{
					...
					if (HandleClient(SockIndex, ServerSock, MaxSockNumber))
					{
#if DEBUG

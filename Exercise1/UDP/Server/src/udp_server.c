...
#define BUFFSIZE 255 /* Max message buffer size */
...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		//Send back with client specific port to sure it can work
		if (sendto(ServerSock, MessageBuffer, ReceivedDataBytes, 0, (struct sockaddr *)
				   &SourceAddrInfo,
				   sizeof(SourceAddrInfo)) != ReceivedDataBytes)
		{
